...
//Mannager类都为单例
public class NetConnetMannager  {
    private Context applicationContext;
    private boolean connectStatus = false;
    ConnectivityManager connectivityManager;
    //使用链表 因为可能多个页面需要来监听
    List<INetConnectListener> iNetConnectListenerList = new LinkedList<>();
    private static NetConnetMannager instance;
    private NetConnetMannager( ){

    }
    public static NetConnetMannager getInstance( ){
        if (instance == null)
            instance = new NetConnetMannager();
        return instance;
    }
